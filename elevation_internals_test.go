...
func TestElevationControl_ConfigureExit(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			IsElevated = func(_ windows.Token) bool { return tt.assertElevated }
			scanfInvoked = false
			ec := NewElevationControl()
			_ec := ec.(*elevationControl)
			got := _ec.ConfigureExit(exitFunction)
			got(0)
			if !exitInvoked {
